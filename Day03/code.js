...
var wire1PathArray = input1.split(",");
var wire2PathArray = input2.split(",");
var boardMap1 = new Map();
var boardMap2 = new Map();
fillBoardMap(wire1PathArray, boardMap1);
fillBoardMap(wire2PathArray, boardMap2);
var intersections = [];

var xValues = boardMap1.keys();
var xValue = xValues.next();
while (!xValue.done) {
  if (boardMap2.has(xValue.value)) {
    boardMap1.get(xValue.value).forEach(yValue => {
      if (boardMap2.get(xValue.value).has(yValue)) {
        intersections.push([xValue.value, yValue]);
      }
    });
  }
  xValue = xValues.next();
...
function fillBoardMap(pathArray, boardMap) {
  for (let i = 0, workX = 0, workY = 0; i < pathArray.length; i++) {
    direction = pathArray[i].substring(0, 1);
    distance = parseInt(pathArray[i].substring(1));

    if (direction == "R") {
      for (let j = 0; j < distance; j++) {
        workX++;
        if (boardMap.has(workX)) {
          boardMap.get(workX).add(workY);
        } else {
          boardMap.set(workX, new Set([workY]));
        }
      }
    } else if (direction == "L") {
      for (let j = 0; j < distance; j++) {
        workX--;
        if (boardMap1.has(workX)) {
          boardMap.get(workX).add(workY);
        } else {
          boardMap.set(workX, new Set([workY]));
        }
